<template>
	<div>
		<ul class="list-group">
			<li v-for="(item, index) in arrayOrdenado" :key="item.id"  @click="aumentar(index)"
...
<script>
	...
	export default {
		...
		computed:{
			...mapState(['frutas']),
			arrayOrdenado(){
				// criterio de orden (a, b)=> b.cantidad - a.cantidad 
				return this.frutas.sort( (a, b)=> b.cantidad - a.cantidad )
			}
		},
		methods: {
			...mapMutations(['aumentar','disminuir','reiniciar'])
		},
